...
export class ProjectService implements IProjectService {
  ...
  public async getProjectRecommendationQuestions() {
    ...
    if (project.queryId) {
      result.status = project.questionsStatus
        ? (RecommendQuestionResultStatus[project.questionsStatus as keyof typeof RecommendQuestionResultStatus] || result.status)
        : result.status;
      result.questions = project.questions || [];
      result.error = project.questionsError as AnalyticsAIError | undefined;
    }
    return result;
  }
  ...
  public getGeneralConnectionInfo(project: Project) {
    return Object.entries(project.connectionInfo).reduce<Record<string, any>>(
      (acc, [key, value]) => {
