...
export class DashboardService implements IDashboardService {
  ...
  public async setDashboardSchedule(
    ...
    try {
      ...
      if (cacheEnabled && schedule && schedule.frequency !== ScheduleFrequencyEnum.NEVER) {
      ...
      logger.error(`Failed to set dashboard schedule: ${errorMessage}`);
      throw error;
    }
  }
  ...
  protected calculateNextRunTime(cronExpression: string): Date | null {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Failed to parse cron expression: ${errorMessage}`);
      return null;
    }
  }
  ...
  public parseCronExpression(dashboard: Dashboard): DashboardSchedule {
    if (!dashboard.scheduleCron) {
      return {
        frequency: dashboard.scheduleFrequency || ScheduleFrequencyEnum.NEVER,
        hour: 0,
        minute: 0,
