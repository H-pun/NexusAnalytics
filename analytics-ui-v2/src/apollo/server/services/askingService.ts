...
import { IThreadRepository, Thread } from '@server/repositories';
import {
    ...
    setInterval(() => {
      const jobs = Object.values(this.tasks).map(
        (threadResponse) => async () => {
          ...
          if (!breakdownDetail) {
            logger.debug(`Job ${threadResponse.id} has no breakdown detail, finished`);
...
export class AskingService implements IAskingService {
  ...
  public async getThreadRecommendationQuestions(
    ...
    if (thread.queryId && thread.questionsStatus) {
      const statusKey = thread.questionsStatus as keyof typeof RecommendQuestionResultStatus;
      res.status = RecommendQuestionResultStatus[statusKey]
        ? RecommendQuestionResultStatus[statusKey]
        : res.status;
      res.questions = thread.questions || [];
      res.error = (thread.questionsError as AnalyticsAIError) || undefined;
    }
    return res;
  }
  ...
  public async adjustThreadResponseChart(
    ...
    const chartSchema = threadResponse.chartDetail?.chartSchema;
    if (!chartSchema) {
      throw new Error(`Thread response ${threadResponseId} has no chart schema`);
    }
  ...
  public async previewBreakdownData(
    ...
    if (!steps) {
      throw new Error(`Thread response ${responseId} has no breakdown steps`);
    }
  ...
  private async getDeployId() {
    ...
    if (!lastDeploy) {
      throw new Error(`No deployment found for project ${id}`);
    }
  ...
  private getThreadRecommendationQuestionsConfig(project: Project) {
    const languageKey = project.language as keyof typeof AnalyticsAILanguage;
    return {
      ...
      configuration: {
        language: (languageKey && AnalyticsAILanguage[languageKey]) || AnalyticsAILanguage.EN,
      },
    };
  }
}
