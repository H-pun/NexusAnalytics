...
export const constructCteSql = (
  ...
  const slicedSteps =
    stepIndex === undefined ? steps : steps.slice(0, stepIndex + 1);
    ...
    setInterval(() => {
      const jobs = Object.values(this.tasks).map(
        (threadResponse) => async () => {
          ...
          if (!breakdownDetail) {
            logger.debug(
              `Job ${threadResponse.id} has no breakdownDetail, finished`,
            );
...
export class AskingService implements IAskingService {
  ...
  public async initialize() {
    ...
    // Initialize breakdown thread responses
    ...
    // Initialize text-based answer thread responses
    // Find thread responses with SQL but no answerDetail or answerDetail with NOT_STARTED status
    const unfinishedAnswerThreadResponses = threadResponses.filter(
      (threadResponse) => {
        if (!threadResponse.sql) return false;
        
        const answerStatus = threadResponse.answerDetail?.status;
        // Include if answerDetail doesn't exist, is null, or is NOT_STARTED
        return (
          !answerStatus ||
          answerStatus === ThreadResponseAnswerStatus.NOT_STARTED
        );
      },
    );
    
    logger.info(
      `Initialization: adding unfinished answer thread responses (total: ${unfinishedAnswerThreadResponses.length}) to background tracker`,
    );
    for (const threadResponse of unfinishedAnswerThreadResponses) {
      // Initialize answerDetail if it doesn't exist or has null status
      if (!threadResponse.answerDetail?.status) {
        const updated = await this.threadResponseRepository.updateOne(
          threadResponse.id,
          {
            answerDetail: {
        ...
        // Use updated response for tracker
        this.textBasedAnswerBackgroundTracker.addTask(updated);
      } else {
  ...
  public async getAskingTaskById(
    ...
    if (!result) {
      throw new Error(`Asking task with id ${id} not found`);
    }
    return result;
  }
  ...
  public async createThread(input: AskingDetailTaskInput): Promise<Thread> {
    ...
    // Extract SQL from trackedAskingResult if not provided directly
    let sql = input.sql;
    if (!sql && input.trackedAskingResult?.response?.[0]?.sql) {
      sql = input.trackedAskingResult.response[0].sql;
    }
    ...
    // if SQL is provided (from TEXT_TO_SQL task or recommendation questions),
    // automatically start generating the text-based answer
    // Refresh thread response to get updated SQL (in case it was updated by bindThreadResponse)
    const refreshedResponse = await this.threadResponseRepository.findOneBy({
      id: threadResponse.id,
    });
    if (refreshedResponse?.sql) {
      await this.generateThreadResponseAnswer(refreshedResponse.id);
    }
  ...
  public async createThreadResponse(
    ...
    return refreshedResponse || threadResponse;
  }
  ...
  public async getAdjustmentTask(
    ...
    const result = await this.adjustmentBackgroundTracker.getAdjustmentResult(
      taskId,
    );
    if (!result) {
      throw new Error(`Adjustment task ${taskId} not found`);
    }
    return result;
  }
  ...
  public async getAdjustmentTaskById(
    ...
    if (!result) {
      throw new Error(`Adjustment task with id ${id} not found`);
    }
    return result;
  }
