...
import { ISqlPairRepository } from '@server/repositories';
...
export class SqlPairService implements ISqlPairService {
  ...
  public async generateQuestions(
    ...
    try {
      const configurations = {
        language: project.language ? (AnalyticsAILanguage[project.language as keyof typeof AnalyticsAILanguage] || AnalyticsAILanguage.EN) : AnalyticsAILanguage.EN,
      };
  ...
  public async createSqlPairs(
    ...
    const errorPairs: AnalyticsAIError[] = [];
    ...
    for (const pairs of chunks) {
      await Promise.allSettled(
        pairs.map(async (pair) => {
          const { queryId } = await this.analyticsAIAdaptor.deploySqlPair(
            projectId,
            { question: pair.question, sql: pair.sql },
          );
          const deployResult = await this.waitUntilSqlPairResult(queryId);
          if (deployResult.error) {
            errorPairs.push(deployResult.error);
          } else {
            // Store the ID of the successfully deployed pair from newPairs
            const newPair = newPairs.find((np) => np.question === pair.question && np.sql === pair.sql);
            if (newPair?.id) {
              successPairIds.push(newPair.id);
            }
          }
        }),
      ).then(async (_result) => {
        if (errorPairs.length > 0) {
          logger.debug(
            `deploy sql pair failed. ${errorPairs.map((err) => err.message).join(', ')}`,
          );
          await tx.rollback();
          if (successPairIds.length > 0) {
          ...
          throw Errors.create(Errors.GeneralErrorCodes.DEPLOY_SQL_PAIR_ERROR, {
            customMessage: errorPairs.map((err) => err.message).join(', '),
          });
        }
      });
    }
  ...
  async editSqlPair(
    ...
    try {
      ...
      logger.error(`edit sql pair failed. ${errorMessage}`);
      await tx.rollback();
      throw Errors.create(Errors.GeneralErrorCodes.DEPLOY_SQL_PAIR_ERROR, {
        customMessage: errorMessage,
      });
  ...
  async deleteSqlPair(projectId: number, sqlPairId: number): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`delete sql pair failed. ${errorMessage}`);
      await tx.rollback();
      throw Errors.create(Errors.GeneralErrorCodes.DEPLOY_SQL_PAIR_ERROR, {
        customMessage: errorMessage,
      });
