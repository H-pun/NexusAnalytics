import { SampleDatasetTable, SampleDatasetColumn } from '@server/data';
import {
...
export class ModelService implements IModelService {
  ...
  public async createCalculatedField(
    ...
    if (!canQuery) {
      ...
      throw Errors.create(Errors.GeneralErrorCodes.INVALID_CALCULATED_FIELD, {
        customMessage: parsedErrorMessage?.message || errorMessage || 'Invalid calculated field',
  ...
  public async batchUpdateColumnProperties(tables: SampleDatasetTable[]) {
    ...
    const transformedColumns = tables.reduce<Array<SampleDatasetColumn & { tableName: string }>>((acc, table) => {
  ...
  public async getCalculatedFieldByRelation(
    ...
    const relatedCalculatedFields = calculatedFields.reduce<ModelColumn[]>((acc, field) => {
      if (!field.lineage) {
        return acc;
      }
  ...
  private generateRelationName(
    ...
    if (!fromColumn || !toColumn) {
      throw new Error(`Column not found for relation between model ${relation.fromModelId} and ${relation.toModelId}`);
    }
