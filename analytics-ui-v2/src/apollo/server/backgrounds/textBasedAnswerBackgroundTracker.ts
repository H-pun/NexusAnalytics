    ...
    setInterval(async () => {
      const jobs = Object.values(this.tasks).map(
        (threadResponse) => async () => {
          ...
          // Only process if status is NOT_STARTED or null/undefined (legacy data)
          ...
          // If status is null/undefined, initialize it to NOT_STARTED first
          if (!status) {
            ...
            // Refresh threadResponse to get updated answerDetail
            const refreshed = await this.threadResponseRepository.findOneBy({
              id: threadResponse.id,
            });
            if (refreshed) {
              Object.assign(threadResponse, refreshed);
            }
          }
