...
export const errorMessages = {
  ...
  // resource not found
  [GeneralErrorCodes.RESOURCE_NOT_FOUND]: 'Resource not found',
};
...
export const create = (
  ...
  const err = new GraphQLError(message, {
    extensions: {
      ...
      service,
      shortMessage:
        (shortMessages[code as keyof typeof shortMessages]) ||
  ...
  if (error instanceof GraphQLError) {
    ...
    return {
      ...
      extensions: {
        code,
        message: error.message,
        shortMessage: (shortMessages[code as keyof typeof shortMessages]) || undefined,
        stacktrace: (error.extensions?.exception as { stacktrace?: string[] })?.stacktrace,
      },
    };
  }
