...
/**
 * Common response handler for App Router API endpoints (route handlers)
 */
export const respondWithRoute = async ({
  statusCode,
  responsePayload,
  projectId,
  apiType,
...
/**
 * Common error handler for App Router API endpoints (route handlers)
 */
export const handleApiErrorRoute = async ({
  error,
  projectId,
  apiType,
  requestPayload,
  ...
  if (error instanceof ApiError && error.code) {
    ...
    responsePayload = { error: error.message || 'Internal server error' };
  }
