...
const encryptor = new Encryptor({
  encryptionPassword: config.encryptionPassword || 'default-password',
  encryptionSalt: config.encryptionSalt || 'default-salt',
});
...
export function encryptConnectionInfo(
  ...
  return dataSource[dataSourceType].sensitiveProps.reduce(
    (acc, prop: string) => {
      const value = (connectionInfo as Record<string, any>)[prop];
      if (value) {
...
export function toIbisConnectionInfo(
  ...
  const dataSourceConfig = dataSource[dataSourceType as keyof typeof dataSource];
  return dataSourceConfig.toIbisConnectionInfo(connectionInfo as any);
}
...
export function toMultipleIbisConnectionInfos(
  ...
  const dataSourceConfig = dataSource[dataSourceType as keyof typeof dataSource];
  if (!dataSourceConfig.toMultipleIbisConnectionInfos) {
    return null;
  }
  return dataSourceConfig.toMultipleIbisConnectionInfos(
