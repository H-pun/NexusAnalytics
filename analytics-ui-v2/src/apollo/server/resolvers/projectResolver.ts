...
export class ProjectResolver {
  ...
  public async startSampleDataset(
    ...
    try {
      ...
      if (!relations) {
        throw new Error(`No relations found for dataset: ${name}`);
      }
  ...
  public async saveDataSource(
    ...
    try {
      ...
      if (type === DataSourceName.DUCKDB) {
        void (connectionInfo as DUCKDB_CONNECTION_INFO);
        await this.buildDuckDbEnvironment(ctx, {
          initSql: (connectionInfo as DUCKDB_CONNECTION_INFO).initSql || '',
          extensions: (connectionInfo as DUCKDB_CONNECTION_INFO).extensions || [],
          configurations: (connectionInfo as DUCKDB_CONNECTION_INFO).configurations || {},
        });
      } else {
      ...
      const errorService = err instanceof Error && (err as any).extensions?.service;
      ctx.telemetry.sendEvent(
        eventName,
        { eventProperties, error: errorMessage },
        errorService,
        false,
      );
      throw err;
  ...
  public async getSchemaChange(_root: any, _arg: any, ctx: IContext) {
    ...
    const unresolvedChanges = Object.keys(resolves).reduce((result: any, key) => {
  ...
  public async resolveSchemaChange(
    ...
    try {
      ...
      const errorService = error instanceof Error && (error as any).extensions?.service;
      ctx.telemetry.sendEvent(
        eventName,
  ...
  private async overwriteModelsAndColumns(
    ...
    const columnValues = selectedTables.flatMap((table) => {
      ...
      if (!model) {
        throw new Error(`Model not found for table: ${table.name}`);
      }
