...
export class ModelResolver {
  ...
  private async handleUpdateModel(
    ...
    if (!model) {
      throw new Error(`Model with id ${args.where.id} not found`);
    }
    ...
    if (!sourceTableColumns) {
      throw new Error(`Source table columns not found for table: ${sourceTableName}`);
    }
    ...
    if (toUpdateColumns.length) {
      for (const { id, sourceColumnName, type } of toUpdateColumns) {
        ...
        if (type.includes('STRUCT')) {
          ...
          if (!sourceColumn) {
            throw new Error(`Source column ${sourceColumnName} not found`);
          }
  ...
  public async createView(_root: any, args: any, ctx: IContext) {
    ...
    if (!response.sql) {
      throw new Error(`SQL not found in thread response ${responseId}`);
    }
