...
import type { IContext } from '@server/types/context';
import { getLogger } from '@server/utils';
import { SchemaChange } from '@server/repositories';
  ...
  implements IDataSourceSchemaDetector
{
  ...
  public async resolveSchemaChange(type: string) {
    ...
    if (schemaChangeType === SchemaChangeType.DELETED_TABLES && changes) {
      // delete models
  ...
  public getAffectedResources(
    ...
    const affectedResources = affectedModels.map((model) => {
      const affectedTable = changes.find(
        (table) => table.name === model.sourceTableName,
      );
      if (!affectedTable) {
        throw new Error(`Table ${model.sourceTableName} not found in changes`);
      }
      const affectedColumns = affectedTable.columns;
        ...
        (result: {
          ...
          const affectedCalculatedFieldsByColumnId = allCalculatedFields.filter(
            (calculatedField) => {
              ...
              return (
                Array.isArray(lineage) && lineage.length > 0 && lineage[lineage.length - 1] === affectedColumn.id
              );
            },
          );
          ...
          const affectedRelationships = modelRelationships
            ...
            .filter((relationship): relationship is RelationInfo => !!relationship);
          ...
          affectedRelationships.forEach((relationship) => {
            ...
            const displayName = models.find(
              (m) => m.referenceName === referenceName,
            )?.displayName;
            ...
            const affectedCalculatedFieldsByRelationshipId =
              allCalculatedFields.filter((calculatedField) => {
                ...
                return Array.isArray(lineage) && lineage.includes(relationship.id);
              });
  ...
  private async getDiffSchema() {
    ...
    const diffSchema = currentSchema.reduce((result: DataSourceSchemaChange, currentTable) => {
      ...
      if (diffColumns.length > 0) {
        const deletedColumnChange: DataSourceSchema = { name: currentTable.name, columns: [] };
        const modifiedColumnChange: DataSourceSchema = { name: currentTable.name, columns: [] };
        ...
        for (const currentColumn of diffColumns) {
          ...
          if (!latestColumn) {
            deletedColumnChange.columns.push({ name: currentColumn.name, type: currentColumn.type });
            continue;
          }
          // If the column is found in the latest schema, it means the column has been modified.
          // save latest column as modified column
          modifiedColumnChange.columns.push({ name: latestColumn.name, type: latestColumn.type });
        }
  ...
  private async addSchemaChange(diffSchema: DataSourceSchemaChange) {
    const getResolveState = (change: DataSourceSchema[] | undefined) => (!!change ? false : undefined);
  ...
  private async getLatestSchema(): Promise<DataSourceSchema[]> {
    const project = await this.ctx.projectRepository.findOneBy({
      ...
      await this.ctx.projectService.getProjectDataSourceTables(project || undefined);
